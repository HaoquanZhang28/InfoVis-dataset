...
`parse.py` is the code for the first step of data cleaning, where we extract the trade data between the top 10 economies from 30 CSV files and output them to a new CSV file. Meanwhile, we convert the country codes to the exact country names. This CSV file is also too large and hard to visualize.   
